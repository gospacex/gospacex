...
func TestGeneratedContent(t *testing.T) {
	...
	t.Run("MicroserviceStandardContent", func(t *testing.T) {
		g := microservice.NewMicroserviceStandardGenerator("test-svc", filepath.Join(testDir, "standard"))
		g.Generate()
	...
	t.Run("DDDStructure", func(t *testing.T) {
		g := microservice.NewMicroserviceDDDGenerator("test-ddd", filepath.Join(testDir, "ddd"))
		g.Generate()
	...
	t.Run("IstioManifests", func(t *testing.T) {
		g := microservice.NewMicroserviceIstioGenerator("test-istio", filepath.Join(testDir, "istio"))
		g.Generate()
	...
	t.Run("ThriftIDL", func(t *testing.T) {
		g := microservice.NewMicroserviceThriftGenerator("test-thrift", filepath.Join(testDir, "thrift"))
		g.Generate()
	...
	t.Run("DatabaseInit", func(t *testing.T) {
		g := common.NewDatabaseIntegrationGenerator(filepath.Join(testDir, "db"), []string{"mysql", "redis"})
		g.Generate()
	...
	t.Run("MiddlewareInit", func(t *testing.T) {
		g := microservice.NewMiddlewareIntegrationGenerator(filepath.Join(testDir, "mw"), []string{"jaeger", "kafka"})
		g.Generate()
	...
	t.Run("DTMPatterns", func(t *testing.T) {
		g := microservice.NewDTMIntegrationGenerator("test-dtm", filepath.Join(testDir, "dtm"))
		g.Generate()
	...
	t.Run("TestStructure", func(t *testing.T) {
		g := common.NewTestIntegrationGenerator("test-svc", filepath.Join(testDir, "test"))
		g.Generate()
	...
	t.Run("Documentation", func(t *testing.T) {
		g := common.NewDocsReleaseGenerator("test-docs", filepath.Join(testDir, "docs"))
		g.Generate()
