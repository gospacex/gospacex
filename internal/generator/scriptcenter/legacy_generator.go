...
func (g *ScriptCenterGenerator) Generate(outputDir string) error {
	...
	g.config.DB = normalizeDBTypes(g.config)

	if g.shouldUseTemplateGenerator() {
		return NewGenerator(g.config).Generate()
	}
	...
	files := map[string]func() (string, error){
		...
		"pkg/logger/logger.go":  g.generateLogger,
	...
	if g.config.MySQLTable != "" {
		...
		crudGen := common.NewCRUDGeneratorWithHandlerType(
			g.config.MySQLHost,
			g.config.MySQLPort,
			g.config.MySQLUser,
...
func (g *ScriptCenterGenerator) shouldUseTemplateGenerator() bool {
	if g.config.MySQLTable != "" {
		return false
	}
	...
	return hasTemplates(filepath.Join("templates", "scriptcenter", mqTypes[0]))
}
...
func (g *ScriptCenterGenerator) generateConfig() (string, error) {
	...
	dbTypesYAML := "[" + strings.Join(normalizeDBTypes(g.config), ", ") + "]"
...
func (g *ScriptCenterGenerator) generateDBFiles(outputDir string) error {
	...
	for _, db := range normalizeDBTypes(g.config) {
		switch db {
		case "mysql":
			files["pkg/database/mysql_init.go"] = g.generateMySQLInitPkg
		case "postgresql":
			files["pkg/database/postgres_init.go"] = g.generatePostgreSQLInitPkg
		case "sqlite":
			files["pkg/database/sqlite_init.go"] = g.generateSQLiteInitPkg
		case "redis":
			files["pkg/database/redis_init.go"] = g.generateRedisInitPkg
		case "elasticsearch":
			files["pkg/database/es_init.go"] = g.generateElasticsearchInitPkg
		case "mongodb":
			files["pkg/database/mongo_init.go"] = g.generateMongoDBInitPkg
		}
	}
