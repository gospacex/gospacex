...
func (g *Generator) generateSnowflake() error {
	templates := map[string]string{
		"pkg/snowflake/snowflake.go":      filepath.Join("pkg", "snowflake", "snowflake.go.tmpl"),
		"pkg/snowflake/snowflake_test.go": filepath.Join("pkg", "snowflake", "snowflake_test.go.tmpl"),
	}
...
func (g *Generator) renderTemplates(templates map[string]string) error {
	for dst, src := range templates {
		srcPath := resolveTemplatePath(src)
		...
		if err := loader.Load(""); err != nil {
			return fmt.Errorf("load template %s: %w", srcPath, err)
		}

		processor := template.NewProcessor(loader)
		if err := processor.RenderFile(srcPath, dstPath, g.cfg); err != nil {
