...
func (tg *TemplateRedisCRUDGenerator) generateModel() error {
	...
	templatePath := resolveTemplatePath("crud", "redis_model.go.tmpl")
...
func (tg *TemplateRedisCRUDGenerator) generateRepositoryInterface() error {
	...
	templatePath := resolveTemplatePath("crud", "redis_repository_interface.go.tmpl")
	content, err := tg.renderTemplate(templatePath, data)
	if err != nil {
		return err
	}
...
func (tg *TemplateRedisCRUDGenerator) generateRepositoryImpl() error {
	...
	templatePath := resolveTemplatePath("crud", "redis_repository_impl.go.tmpl")
	content, err := tg.renderTemplate(templatePath, data)
	if err != nil {
		return err
	}
...
func (tg *TemplateRedisCRUDGenerator) generateService() error {
	...
	templatePath := resolveTemplatePath("crud", "redis_service.go.tmpl")
	content, err := tg.renderTemplate(templatePath, data)
	if err != nil {
		return err
	}
...
func (tg *TemplateRedisCRUDGenerator) generateDALInit() error {
	...
	templatePath := resolveTemplatePath("crud", "redis_dal_init.go.tmpl")
	content, err := tg.renderTemplate(templatePath, data)
	if err != nil {
		return err
	}
