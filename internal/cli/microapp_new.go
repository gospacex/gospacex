...
var (
	...
	microAppRegistryAddr string  // 解析后的注册中心地址: host:port
	microAppDBURL        string   // 数据库 URL 格式: mysql://user:pass@host:port/dbname?tables=t1,t2&join=t1.id=t2.tid:1t1
	microAppESURL        string   // Elasticsearch URL: elasticsearch://127.0.0.1:9200?index=products&user=elastic&pwd=pass
	microAppMQURL        string   // MQ URL 格式: kafka://127.0.0.1:9092?topic=events&group=myshop | rabbitmq://... | rocketmq://...
	microAppMQType      string   // MQ 类型: kafka|rabbitmq|rocketmq
	microAppMQHost      string   // MQ host
	microAppMQPort      string   // MQ port
	microAppMQTopic     string   // MQ topic
	microAppMQUser      string   // MQ user (for rabbitmq)
	microAppMQPassword  string   // MQ password (for rabbitmq)
	microAppMQGroup     string   // MQ consumer group (for kafka)
	microAppTraceURL    string   // 链路追踪 URL: jaeger://127.0.0.1:14268?service=xxx
	microAppTraceHost   string   // 链路追踪 host
	microAppTracePort   string   // 链路追踪 port
	microAppTraceSvcName string   // 链路追踪服务名

	// ES 参数
	microAppESHost     string // ES host
	microAppESPort     string // ES port
	microAppESIndex    string // ES index
	microAppESUser     string // ES user
	microAppESPassword string // ES password
)
...
// parseRegistryURL 解析注册中心 URL
// 格式: consul://host:port | etcd://host:port | direct
func parseRegistryURL(regURL string) error {
	if regURL == "direct" {
		microAppRegister = "direct"
		return nil
	}
	...
	// 提取 host:port 地址
	if u.Host != "" {
		microAppRegistryAddr = u.Host
	} else if u.Path != "" {
		microAppRegistryAddr = u.Path
	}
	switch u.Scheme {
	case "consul":
		microAppRegister = "consul"
	case "etcd":
		microAppRegister = "etcd"
	default:
...
// parseDBURL 解析数据库 URL
// 格式: mysql://user:pass@host:port/dbname?tables=t1,t2&join=t1.id=t2.tid:1t1
func parseDBURL(dbURL string) error {
	u, err := url.Parse(dbURL)
	if err != nil {
		return fmt.Errorf("invalid db URL: %w", err)
	}
	if u.Scheme != "mysql" && u.Scheme != "postgres" {
	...
	microAppDBHost, microAppDBPort, _ = splitHostPort(u.Host)
	microAppDBUser = user
	microAppDBPassword = password
	microAppDBName = dbname
	return nil
}

// splitHostPort 解析 host:port 格式
...
// parseESURL 解析 Elasticsearch URL
// 格式: elasticsearch://host:9200?index=xxx&user=xxx&pwd=xxx
func parseESURL(esURL string) error {
	u, err := url.Parse(esURL)
	if err != nil {
		return fmt.Errorf("invalid es URL: %w", err)
	}
	if u.Scheme != "elasticsearch" {
		return fmt.Errorf("unsupported es scheme: %s", u.Scheme)
	}
	// 提取 host:port
	host, port, _ := splitHostPort(u.Host)
	microAppESHost = host
	microAppESPort = port
	// 提取 query 参数: index, user, pwd
	q := u.Query()
	microAppESIndex = q.Get("index")
	microAppESUser = q.Get("user")
	microAppESPassword = q.Get("pwd")
	return nil
}

// parseMQURL 解析消息队列 URL
// 格式: kafka://host:9092?topic=xxx&group=xxx | rabbitmq://host:5672?user=xxx&pwd=xxx | rocketmq://host:9876?topic=xxx
func parseMQURL(mqURL string) error {
	...
	case "kafka", "rabbitmq", "rocketmq":
		...
		// 解析 query 参数
		q := u.Query()
		microAppMQTopic = q.Get("topic")
		microAppMQGroup = q.Get("group")
		microAppMQUser = q.Get("user")
		microAppMQPassword = q.Get("pwd")
	default:
		return fmt.Errorf("unsupported mq scheme: %s", u.Scheme)
	}
	return nil
}

// parseTraceURL 解析链路追踪 URL
// 格式: jaeger://host:14268?service=xxx
func parseTraceURL(traceURL string) error {
	u, err := url.Parse(traceURL)
	if err != nil {
		return fmt.Errorf("invalid trace URL: %w", err)
	}
	if u.Scheme != "jaeger" {
		return fmt.Errorf("unsupported trace scheme: %s", u.Scheme)
	}
	// 解析 host:port
	host, port, _ := splitHostPort(u.Host)
	microAppTraceHost = host
	microAppTracePort = port
	// 解析 query 参数: service
	q := u.Query()
	microAppTraceSvcName = q.Get("service")
	return nil
}
...
type ColumnInfo struct {
	Name       string // 字段名
	Type       string // MySQL DATA_TYPE
	TypeFull   string // MySQL COLUMN_TYPE（含精度，如 "int unsigned"）
...
func readTableSchema(host, port, user, password, dbName, tableName string) ([]ColumnInfo, error) {
	...
	for rows.Next() {
		...
		col.Type = colType.String // DATA_TYPE
	col.TypeFull = colTypeFull.String // COLUMN_TYPE (含精度，如 "int unsigned")
...
func applyInteractiveConfig(cfg *InteractiveConfig) error {
	...
	// 解析 URL 化参数
	if microAppRegistry != "" {
		if err := parseRegistryURL(microAppRegistry); err != nil {
			return fmt.Errorf("invalid --registry URL: %w", err)
		}
	}
	if microAppDBURL != "" {
		if err := parseDBURL(microAppDBURL); err != nil {
			return fmt.Errorf("invalid --db-url: %w", err)
		}
	}
	if microAppESURL != "" {
		if err := parseESURL(microAppESURL); err != nil {
			return fmt.Errorf("invalid --es-url: %w", err)
		}
	}
	if microAppMQURL != "" {
		if err := parseMQURL(microAppMQURL); err != nil {
			return fmt.Errorf("invalid --mq-url: %w", err)
		}
	}
	if microAppTraceURL != "" {
		if err := parseTraceURL(microAppTraceURL); err != nil {
			return fmt.Errorf("invalid --trace-url: %w", err)
		}
	}
...
func runNewMicroAppWithFlags() error {
	...
	if hasDBTable {
		...
		if microAppIDL == "thrift" {
			...
			genProtoFilesFromSchema(projectDir, moduleTables, joinConfigs)
		}
		...
		genMQ(projectDir)
		genUtils(projectDir)
		if microAppOtel || microAppTraceURL != "" {
			genTracing(projectDir)
		}
		if microAppESURL != "" {
			genESConfig(projectDir, microAppBFFName, microAppModules)
		}

		// 根据 HTTP 框架选择 BFF 生成函数
		if microAppHTTP == "hertz" {
			genBFFHertzFromSchema(projectDir, microAppBFFName, moduleTables, microAppProtocol, joinConfigs)
		} else {
			genBFFFromSchema(projectDir, microAppBFFName, moduleTables, microAppProtocol, joinConfigs)
		}
		...
		for i, m := range microAppModules {
			...
			port := 8001 + i
			genSrvMainAndConfig(projectDir, m, port, tables, tableNames, joinConfigs)
		}
		...
		for _, m := range microAppModules {
			...
			for _, tbl := range tables {
				// 连表模式下，右表（辅助表）不生成独立的 model/repo/service/handler
	...
	// gRPC 协议默认生成基础 UnaryInterceptor（即使未指定 --middleware）
	if microAppProtocol != "kitex" {
		genGRPCInterceptor(projectDir, microAppModules)
	}
	...
	// 生成 ES 支持（如果 --es-url 启用）
	if microAppESURL != "" {
		genESConfig(projectDir, microAppBFFName, microAppModules)
	}

	// 生成 MQ 支持（如果 --mq-url 启用）
	if microAppMQURL != "" {
		genMQConfig(projectDir, microAppBFFName, microAppModules)
	}

	// 生成链路追踪支持（如果 --trace-url 启用）
	if microAppTraceURL != "" {
		genTraceConfig(projectDir, microAppBFFName, microAppModules)
	}
...
// genMQ 生成消息队列框架代码
func genMQ(projectDir string) {
	if microAppMQURL == "" {
		return
	}

	mqDir := filepath.Join(projectDir, "pkg", "mq")
	os.MkdirAll(mqDir, 0755)

	switch microAppMQType {
	case "kafka":
		genKafkaMQ(mqDir)
	case "rabbitmq":
		// genRabbitMQ(mqDir) // TODO: implement rabbitmq generator
	case "rocketmq":
		// genRocketMQ(mqDir) // TODO: implement rocketmq generator
	}
}

// genKafkaMQ 生成 Kafka 客户端代码
func genKafkaMQ(mqDir string) {
	producer := `package mq
...
`
	os.WriteFile(filepath.Join(mqDir, "producer.go"), []byte(producer), 0644)
...
func NewKafkaConsumer(brokers []string, groupID, topic string, handler func(msg []byte) error) (*KafkaConsumer, error) {
	config := sarama.NewConfig()
	...
	return &KafkaConsumer{consumer: consumer, topic: topic, handler: &consumerHandler{handler: handler}}, nil
}
...
`
	os.WriteFile(filepath.Join(mqDir, "consumer.go"), []byte(consumer), 0644)
...
`
	os.WriteFile(filepath.Join(mqDir, "config.go"), []byte(config), 0644)
}

// genRabbitMQ 生成 RabbitMQ 客户端代码
func genRabbitMQ(mqDir string) {
	producer := `package mq
...
func NewRabbitMQProducer(url, topic string) (*RabbitMQProducer, error) {
	...
	return &RabbitMQProducer{conn: conn, channel: ch, topic: topic}, nil
}

func (p *RabbitMQProducer) Publish(ctx context.Context, body []byte) error {
	return p.channel.PublishWithContext(ctx, "", p.topic, false, false,
		amqp.Publishing{Body: body, DeliveryMode: amqp.Persistent})
}
...
func NewRabbitMQConsumer(url, topic string) (*RabbitMQConsumer, error) {
	conn, err := amqp.Dial(url)
	if err != nil {
		return nil, fmt.Errorf("connect rabbitmq failed: %w", err)
	}
	...
	return &RabbitMQConsumer{conn: conn, channel: ch, topic: topic}, nil
}

func (c *RabbitMQConsumer) Consume(ctx context.Context, handler func([]byte) error) error {
	msgs, err := c.channel.Consume(c.topic, "", false, false, false, false, nil)
	if err != nil {
...
// genRocketMQ 生成 RocketMQ 客户端代码
func genRocketMQ(mqDir string) {
	producer := `package mq
...
func NewRocketMQProducer(endpoints, topic string) (*RocketMQProducer, error) {
	return &RocketMQProducer{topic: topic}, nil
}

func (p *RocketMQProducer) SendMessage(ctx context.Context, body []byte) error {
	fmt.Printf("RocketMQ send to topic %s: %s\n", p.topic, string(body))
	return nil
}
...
func NewRocketMQConsumer(endpoints, groupID, topic string, handler func([]byte) error) (*RocketMQConsumer, error) {
	return &RocketMQConsumer{topic: topic}, nil
}

func (c *RocketMQConsumer) Start(ctx context.Context) error {
	fmt.Printf("RocketMQ consumer started for topic %s\n", c.topic)
	<-ctx.Done()
	return ctx.Err()
}
...
func genTracing(projectDir string) {
	// 确定模板目录
	srcDir := filepath.Join(getTemplatesDir(), "micro-app", "pkg", "tracing")
	if _, err := os.Stat(srcDir); os.IsNotExist(err) {
		fmt.Printf("Warning: tracing templates not found in %s\n", srcDir)
		return
	}
	dstDir := filepath.Join(projectDir, "pkg", "tracing")
	os.MkdirAll(dstDir, 0755)

	tmplFiles := []string{
		"tracing.go.tmpl",
		"interceptors.go.tmpl",
		"middleware.go.tmpl",
	}

	for _, tmplFile := range tmplFiles {
		srcPath := filepath.Join(srcDir, tmplFile)
		if _, err := os.Stat(srcPath); os.IsNotExist(err) {
			fmt.Printf("Warning: tracing template %s not found\n", tmplFile)
			continue
		}

		tmplStr, err := os.ReadFile(srcPath)
		if err != nil {
			fmt.Printf("ERROR reading tracing template %s: %v\n", tmplFile, err)
			continue
		}

		outFile := strings.TrimSuffix(tmplFile, ".tmpl")
		outPath := filepath.Join(dstDir, outFile)

		content, err := executeTemplate(string(tmplStr), map[string]interface{}{"AppName": microAppName})
		if err != nil {
			fmt.Printf("ERROR executing tracing template %s: %v\n", tmplFile, err)
			continue
		}
		if err := os.WriteFile(outPath, []byte(content), 0644); err != nil {
			fmt.Printf("ERROR writing tracing file %s: %v\n", outFile, err)
			continue
		}
		fmt.Printf("  Generated: pkg/tracing/%s\n", outFile)
	}
}
...
func genBFF(projectDir, bffName string, modules []string, protocol string) {
	...
	// config.yaml
	var bffCfg string
	registryAddr := microAppRegistryAddr
	if registryAddr == "" {
	...
	if microAppMQURL != "" {
		bffCfg += "\nmq:\n"
		bffCfg += fmt.Sprintf("  type: %s\n", microAppMQType)
		if microAppMQType == "kafka" {
			bffCfg += fmt.Sprintf("  brokers:\n    - %s:%s\n", microAppMQHost, microAppMQPort)
		} else if microAppMQType == "rabbitmq" {
			bffCfg += fmt.Sprintf("  url: %s://%s:%s@%s:%s\n", microAppMQType, microAppMQUser, microAppMQPassword, microAppMQHost, microAppMQPort)
		} else if microAppMQType == "rocketmq" {
			bffCfg += fmt.Sprintf("  endpoints: %s:%s\n", microAppMQHost, microAppMQPort)
		}
		bffCfg += fmt.Sprintf("  topic: %s\n", microAppMQTopic)
		if microAppMQGroup != "" {
			bffCfg += fmt.Sprintf("  group_id: %s\n", microAppMQGroup)
		}
	}
	// 添加 tracing 配置
	if microAppTraceURL != "" {
		bffCfg += "\ntracing:\n"
		bffCfg += fmt.Sprintf("  enabled: true\n")
		bffCfg += fmt.Sprintf("  endpoint: %s:%s\n", microAppTraceHost, microAppTracePort)
		bffCfg += fmt.Sprintf("  service_name: %s\n", microAppTraceSvcName)
	}
...
func genMicroservice(projectDir, module string, port int, protocol string) {
	...
	if microAppRegister != "" {
		cfg = fmt.Sprintf("server:\n  host: 0.0.0.0\n  port: %d\n\ndatabase:\n  host: %s\n  port: %s\n  user: %s\n  password: %s\n  database: %s\n\nregistry:\n  type: %s\n  address: %s\n", port, microAppDBHost, microAppDBPort, microAppDBUser, microAppDBPassword, microAppDBName, microAppRegister, registryAddr)
	} else {
	...
	if microAppMQURL != "" {
		cfg += "\nmq:\n"
		cfg += fmt.Sprintf("  type: %s\n", microAppMQType)
		if microAppMQType == "kafka" {
			cfg += fmt.Sprintf("  brokers:\n    - %s:%s\n", microAppMQHost, microAppMQPort)
		} else if microAppMQType == "rabbitmq" {
			cfg += fmt.Sprintf("  url: %s://%s:%s@%s:%s\n", microAppMQType, microAppMQUser, microAppMQPassword, microAppMQHost, microAppMQPort)
		} else if microAppMQType == "rocketmq" {
			cfg += fmt.Sprintf("  endpoints: %s:%s\n", microAppMQHost, microAppMQPort)
		}
		cfg += fmt.Sprintf("  topic: %s\n", microAppMQTopic)
		if microAppMQGroup != "" {
			cfg += fmt.Sprintf("  group_id: %s\n", microAppMQGroup)
		}
	}
...
func genScripts(projectDir string, moduleTables ModuleTables, joinConfigs []*JoinConfig) {
	...
	// 处理所有表（连表模式下仍然处理所有表，第一张表做主表进行左连接）
...
func genProtoFilesFromSchema(projectDir string, moduleTables ModuleTables, joinConfigs []*JoinConfig) {
	...
	for moduleName, tables := range moduleTables {
		// 处理所有表（连表模式下第一张表做主表进行左连接，但所有表都生成 proto）
		tablesToProcess := tables
		for _, table := range tablesToProcess {
				// 连表模式下，右表（辅助表）不生成独立的 proto 文件
...
func genBFFFromSchema(projectDir, bffName string, moduleTables ModuleTables, protocol string, joinConfigs []*JoinConfig) {
...
`), 0644)
	...
	for moduleName, tables := range moduleTables {
		upper := toPascalCase(moduleName)
		for _, table := range tables {
			// 连表模式下，右表（辅助表）不生成独立的 handler 和 rpc_client
			if isJoinTable(table.TableName, joinConfigs) {
				continue
			}
			columns := table.Columns
...
// isJoinTable 检查表是否为联表中的右表（辅助表），右表不生成独立的 CRUD
func isJoinTable(tableName string, joinConfigs []*JoinConfig) bool {
	for _, jc := range joinConfigs {
		if jc.RightTable == tableName {
			return true
		}
	}
	return false
...
func genSrvMainAndConfig(projectDir, module string, port int, tables []TableInfo, tableNames []string, joinConfigs []*JoinConfig) {
	// 处理所有表（连表模式下第一张表做主表进行左连接，但所有表都注册 service）
	tablesToProcess := tables
	...
	if len(tablesToProcess) > 0 {
		tableName = tablesToProcess[0].TableName
		entityName := tableToEntityName(tableName, tableNames)
		alias := toCamelCase(entityName)
		upperEntity := toPascalCase(entityName)
		if len(tablesToProcess) == 1 {
	...
	if microAppESURL != "" {
		cfg += fmt.Sprintf("\nelasticsearch:\n  addresses:\n    - %s:%s\n  username: %s\n  password: %s\n  index: %s\n",
			microAppESHost, microAppESPort, microAppESUser, microAppESPassword, microAppESIndex)
	}
	// 添加链路追踪配置
	if microAppTraceURL != "" || microAppOtel {
		cfg += fmt.Sprintf("\ntracing:\n  enabled: true\n  endpoint: %s:%s\n  service_name: %s\n",
			microAppTraceHost, microAppTracePort, microAppName+"-"+module)
	}
	os.WriteFile(filepath.Join(projectDir, toSrvDirName(module), "configs", "config.yaml"), []byte(cfg), 0644)
}
...
func genBFFHertzFromSchema(projectDir, bffName string, moduleTables ModuleTables, protocol string, joinConfigs []*JoinConfig) {
	// main.go
...
func init() {
	...
	newMicroAppCmd.Flags().StringVar(&microAppDBURL, "db-url", "", "数据库 URL: mysql://user:pass@host:port/dbname?tables=t1,t2&join=t1.id=t2.tid:1t1")
	newMicroAppCmd.Flags().StringVar(&microAppESURL, "es-url", "", "Elasticsearch URL: elasticsearch://host:9200?index=xxx&user=xxx&pwd=xxx")
	newMicroAppCmd.Flags().StringVar(&microAppESURL, "es", "", "Elasticsearch URL（简写）")
	newMicroAppCmd.Flags().StringVar(&microAppMQURL, "mq-url", "", "消息队列 URL: kafka://host:9092?topic=xxx | rabbitmq://host:5672?user=xxx&pwd=xxx | rocketmq://host:9876?topic=xxx")
	newMicroAppCmd.Flags().StringVar(&microAppMQURL, "mq", "", "消息队列 URL（简写）")
	newMicroAppCmd.Flags().StringVar(&microAppTraceURL, "trace-url", "", "链路追踪 URL: jaeger://host:14268?service=xxx")
	newMicroAppCmd.Flags().StringVar(&microAppTraceURL, "trace", "", "链路追踪 URL（简写）")
}
...
func genJoinCode(projectDir string, modules []string, joinCfg *JoinConfig, allTableCols map[string][]ColumnInfo) {
	...
	// 左表：优先精确匹配，其次用 contains 匹配
	leftMatched := false
	for _, actualTn := range actualTableNames {
		if actualTn == joinCfg.LeftTable {
			leftTableName = actualTn
			leftMatched = true
			break
		}
	}
	// 如果精确匹配没找到，用 contains 匹配
	if !leftMatched {
		for _, actualTn := range actualTableNames {
			if strings.Contains(actualTn, joinCfg.LeftTable) {
				leftTableName = actualTn
				leftMatched = true
				break
			}
	...
	// 如果左表没有匹配，使用第一个实际表名
	if !leftMatched && len(actualTableNames) > 0 {
		leftTableName = actualTableNames[0]
	}

	// 右表：优先精确匹配，其次用 contains 匹配（但要排除已匹配的左表）
	rightMatched := false
	for _, actualTn := range actualTableNames {
		if actualTn == joinCfg.RightTable {
			rightTableName = actualTn
			rightMatched = true
			break
		}
	}
	// 如果精确匹配没找到，用 contains 匹配（排除左表）
	if !rightMatched {
		for _, actualTn := range actualTableNames {
			if actualTn != leftTableName && strings.Contains(actualTn, joinCfg.RightTable) {
				rightTableName = actualTn
				rightMatched = true
				break
			}
	...
	// 如果右表没有匹配，使用第二个实际表名（排除左表）
	if !rightMatched {
		rightIsAux = true
		for _, actualTn := range actualTableNames {
			if actualTn != leftTableName {
				rightTableName = actualTn
				break
			}
		}
	...
	// 生成联表 Model（扁平结构）
	genJoinModel(projectDir, leftModule, leftUpper, rightUpper, leftTableName, rightTableName, joinCfg, allTableCols)

	// 生成联表 Repository
	genJoinRepository(projectDir, leftModule, leftUpper, rightModule, rightUpper, joinCfg, rightIsAux, leftTableName, rightTableName, allTableCols)

	// 生成联表 Service
	genJoinService(projectDir, leftModule, leftUpper, rightModule, rightUpper, joinCfg, rightIsAux, leftTableName, rightTableName, allTableCols)
...
// appendJoinProtoRPC 修改主表的 proto 文件，追加 join 查询 RPC
func appendJoinProtoRPC(projectDir, leftModule, leftUpper, rightUpper, leftTableName, rightTableName string, joinCfg *JoinConfig, allTableCols map[string][]ColumnInfo) {
	// 找到主表对应的 proto 文件
	leftProtoFile := toCamelCaseFile(leftTableName)
	if leftProtoFile == "" {
		leftProtoFile = leftModule
	}
	protoPath := filepath.Join(projectDir, "common", "idl", leftProtoFile+".proto")
	
	// 读取现有 proto 文件
	protoContent, err := os.ReadFile(protoPath)
	if err != nil {
		fmt.Printf("WARNING: cannot read proto file %s: %v\n", protoPath, err)
		return
	}
	
	// 获取右表列信息用于生成 join 响应
	var rightColumns []ColumnInfo
	for tn, cols := range allTableCols {
		if tn == rightTableName {
			rightColumns = cols
		}
	}
	...
	pkField := getPrimaryKeyField(leftColumns)
	...
	for _, col := range rightColumns {
		...
		sb.WriteString(fmt.Sprintf("  %s %s = %d; // %s\n", col.ProtoType, fieldName, protoIndex, aliasName))
		protoIndex++
	}
	sb.WriteString("}\n")
	...
	// 在文件末尾追加 join 消息定义
	protoContentStr = strings.TrimRight(protoContentStr, "\n") + sb.String()
	
	// 写回文件
	os.WriteFile(protoPath, []byte(protoContentStr), 0644)
	fmt.Printf("  Updated proto: %s (added %s RPC)\n", leftProtoFile+".proto", joinRPCName)
}

// genJoinModel 生成联表查询的扁平 Model 结构（合并左右表所有字段）
func genJoinModel(projectDir, leftModule, leftUpper, rightUpper, leftTableName, rightTableName string, joinCfg *JoinConfig, allTableCols map[string][]ColumnInfo) {
	// 获取左右表的列信息
	var leftColumns, rightColumns []ColumnInfo
	for tn, cols := range allTableCols {
		if tn == leftTableName {
			leftColumns = cols
		}
	...
	if len(leftColumns) == 0 || len(rightColumns) == 0 {
		fmt.Printf("WARNING: cannot find columns for join model (left=%s, right=%s)\n", leftTableName, rightTableName)
		return
	}
	
	// 转换左表列：ColumnInfo -> map with GoName, GoType, ColumnName, Comment
	// 注意：join model 所有字段类型都使用 string（与 myshop1 基准一致）
	type colForTmpl struct {
		GoName     string
		GoType     string
		ColumnName string
		Comment    string
	...
	var leftForTmpl, rightForTmpl []colForTmpl
	for _, c := range leftColumns {
		leftForTmpl = append(leftForTmpl, colForTmpl{
			GoName:     toProtoGoFieldName(c.Name),
			GoType:     joinColType(c),
			ColumnName: c.Name,
			Comment:    c.Comment,
		})
	}
	...
	for _, c := range rightColumns {
		rightForTmpl = append(rightForTmpl, colForTmpl{
			GoName:     rightPrefix + toProtoGoFieldName(c.Name),
			GoType:     joinColType(c),
			ColumnName: rightAlias + "_" + c.Name,
			AliasName:  rightAlias + "_" + c.Name,
			Comment:    c.Comment,
		})
	}
	...
	data := map[string]interface{}{
		"PackageName":   "model",
		"LeftEntity":    leftUpper,
		"RightEntity":   rightUpper,
		"LeftUpper":     leftUpper,
		"RightUpper":    rightUpper,
		"LeftTable":     leftTableName,
		"RightTable":    rightTableName,
		"RightAlias":    "store_product_attr", // 模板注释用（与 myshop1 一致）
		"LeftColumns":   leftForTmpl,
		"RightColumns":  rightForTmpl,
	}

	// 渲染模板
	tmplPath := filepath.Join(getTemplatesDir(), "micro-app", "join", "model", "join.go.tmpl")
	tmplBytes, err := os.ReadFile(tmplPath)
	if err != nil {
		fmt.Printf("ERROR reading join model template: %v\n", err)
		return
	}
	result, err := executeTemplate(string(tmplBytes), data)
	if err != nil {
		fmt.Printf("ERROR executing join model template: %v\n", err)
		return
	}

	// 写入文件
	modelDir := filepath.Join(projectDir, toSrvDirName(leftModule), "internal", "model")
	os.MkdirAll(modelDir, 0755)
	fileName := fmt.Sprintf("join%s%s.go", leftUpper, rightUpper)
	os.WriteFile(filepath.Join(modelDir, fileName), []byte(result), 0644)
	fmt.Printf("  Generated: %s/internal/model/%s\n", toSrvDirName(leftModule), fileName)
}
...
func genJoinRepository(projectDir, leftModule, leftUpper, rightModule, rightUpper string, joinCfg *JoinConfig, rightIsAux bool, leftTableName, rightTableName string, allTableCols map[string][]ColumnInfo) {
	...
	case "1t1":
		...
		findResult, err := executeTemplate(string(findTmplBytes), map[string]interface{}{
			...
			"LeftColumns":  allTableCols[leftTableName],
	...
	// 追加到主 Repository 文件 instead of creating separate join file
	// 使用 entity 名而不是表名来构建路径（因为表名可能和 entity 名不同，如 store_product -> product）
	// 先获取 allTableCols 中所有实际的表名
	var actualTableNames []string
	for tn := range allTableCols {
		actualTableNames = append(actualTableNames, tn)
	}
	...
	if err != nil {
		...
		if err == nil {
			mainRepoPath = altPath
			fmt.Printf("  DEBUG: using alternative path: %s\n", altPath)
		}
	}
	if err == nil {
...
func genJoinService(projectDir, leftModule, leftUpper, rightModule, rightUpper string, joinCfg *JoinConfig, rightIsAux bool, leftTableName, rightTableName string, allTableCols map[string][]ColumnInfo) {
	...
	if err != nil {
		fmt.Printf("Error reading method template: %v\n", err)
		return
	}
	methodResult, err := executeTemplate(string(methodTmplBytes), map[string]interface{}{
		"LeftUpper":  leftUpper,
		"RightUpper": rightUpper,
	})
	if err != nil {
		fmt.Printf("Error executing method template: %v\n", err)
		return
	}

	if _, err := os.Stat(mainSvcPath); err == nil {
		// 主 Service 文件已存在，追加 join 方法
		existingContent, err := os.ReadFile(mainSvcPath)
		if err != nil {
			fmt.Printf("Error reading existing service file: %v\n", err)
			return
		}
		...
		headerTmplBytes, err := os.ReadFile(headerTmplPath)
		if err != nil {
			fmt.Printf("Error reading header template: %v\n", err)
			return
		}
		headerResult, err := executeTemplate(string(headerTmplBytes), map[string]interface{}{
			"LeftImport":     leftImport,
...
// genGRPCInterceptor 为 gRPC 协议生成基础 UnaryInterceptor（默认生成，不依赖 --middleware）
func genGRPCInterceptor(projectDir string, modules []string) {
	for _, module := range modules {
		srvInterceptorDir := filepath.Join(projectDir, toSrvDirName(module), "internal", "interceptor")
		os.MkdirAll(srvInterceptorDir, 0755)

		baseInterceptorPath := filepath.Join(getTemplatesDir(), "micro-app", "srv", "interceptor", "grpc_interceptor.go.tmpl")
		if _, err := os.Stat(baseInterceptorPath); err == nil {
			interceptorStr, err := os.ReadFile(baseInterceptorPath)
			if err == nil {
				ot := microAppOtel || microAppTraceURL != ""
				interceptorGo, err := executeTemplate(string(interceptorStr), map[string]interface{}{
					"AppName": microAppName,
					"Module":  module,
					"Otel":    ot,
				})
				if err == nil {
					os.WriteFile(filepath.Join(srvInterceptorDir, "grpc_interceptor.go"), []byte(interceptorGo), 0644)
					fmt.Printf("  Generated SRV base interceptor: %s/internal/interceptor/grpc_interceptor.go\n", toSrvDirName(module))
				}
			}
		}
	}
...
// genESConfig 生成 Elasticsearch 支持
func genESConfig(projectDir, bffName string, modules []string) {
	fmt.Println("Generating Elasticsearch support...")

	// 生成 es client 到 pkg/elasticsearch/
	tmplPath := filepath.Join(getTemplatesDir(), "micro-app", "pkg", "elasticsearch", "client.go.tmpl")
	tmplStr, err := os.ReadFile(tmplPath)
	if err != nil {
		fmt.Printf("ERROR reading elasticsearch client template: %v\n", err)
		return
	}

	esGo, err := executeTemplate(string(tmplStr), map[string]interface{}{
		"AppName": microAppName,
	})
	if err != nil {
		fmt.Printf("ERROR executing elasticsearch client template: %v\n", err)
		return
	}
	os.MkdirAll(filepath.Join(projectDir, "pkg", "elasticsearch"), 0755)
	os.WriteFile(filepath.Join(projectDir, "pkg", "elasticsearch", "client.go"), []byte(esGo), 0644)
	fmt.Println("  Generated: pkg/elasticsearch/client.go")

	// 更新 config.yaml 添加 elasticsearch 配置段
	esYaml := fmt.Sprintf("\n# Elasticsearch 配置\nelasticsearch:\n  addresses:\n    - %s:%s\n  username: %s\n  password: %s\n  index: %s\n",
		microAppESHost, microAppESPort, microAppESUser, microAppESPassword, microAppESIndex)
	...
	bffCfgPath := filepath.Join(projectDir, toBffDirName(bffName), "configs", "config.yaml")
	if existing, err := os.ReadFile(bffCfgPath); err == nil {
		os.WriteFile(bffCfgPath, append(existing, []byte(esYaml)...), 0644)
		fmt.Printf("  Updated BFF config: %s/configs/config.yaml (added elasticsearch section)\n", toBffDirName(bffName))
	}

	// 更新 pkg/config/config.go 添加 ESConfig 字段（SRV config.yaml 由 genSrvMainAndConfig 处理）
	configGoPath := filepath.Join(projectDir, "pkg", "config", "config.go")
	if content, err := os.ReadFile(configGoPath); err == nil {
		cfgStr := string(content)
		// 在 Config struct 中添加 Elasticsearch 字段（如果不存在）
		if !strings.Contains(cfgStr, "Elasticsearch ElasticsearchConfig") {
			cfgStr = strings.Replace(cfgStr,
				"Registry RegistryConfig `yaml:\"registry\"`",
		...
		// 在 TracingConfig 之后添加 ElasticsearchConfig struct（如果不存在）
		if !strings.Contains(cfgStr, "type ElasticsearchConfig struct") {
			cfgStr = strings.Replace(cfgStr,
				"ServiceName string `yaml:\"service_name\"`\n}\n",
		...
		os.WriteFile(configGoPath, []byte(cfgStr), 0644)
		fmt.Println("  Updated: pkg/config/config.go (added ElasticsearchConfig)")
	}
}

// genMQConfig 生成消息队列支持
func genMQConfig(projectDir, bffName string, modules []string) {
	fmt.Println("Generating MQ support...")

	mqDir := filepath.Join(projectDir, "pkg", "mq")
	os.MkdirAll(mqDir, 0755)

	tmplPath := filepath.Join(getTemplatesDir(), "micro-app", "pkg", "mq", "client.go.tmpl")
	if tmplStr, err := os.ReadFile(tmplPath); err == nil {
		mqGo, err := executeTemplate(string(tmplStr), map[string]interface{}{"AppName": microAppName})
		if err == nil {
			os.WriteFile(filepath.Join(mqDir, "client.go"), []byte(mqGo), 0644)
			fmt.Println("  Generated: pkg/mq/client.go")
		}
	}

	var tmplPathMQ string
	switch microAppMQType {
	case "kafka":
		tmplPathMQ = filepath.Join(getTemplatesDir(), "micro-app", "pkg", "mq", "kafka.go.tmpl")
	case "rabbitmq":
		tmplPathMQ = filepath.Join(getTemplatesDir(), "micro-app", "pkg", "mq", "rabbitmq.go.tmpl")
	case "rocketmq":
		tmplPathMQ = filepath.Join(getTemplatesDir(), "micro-app", "pkg", "mq", "rocketmq.go.tmpl")
	default:
		fmt.Printf("  WARNING: unknown mq type: %s\n", microAppMQType)
		return
	}

	if tmplStr, err := os.ReadFile(tmplPathMQ); err == nil {
		mqGo, err := executeTemplate(string(tmplStr), map[string]interface{}{"AppName": microAppName})
		if err == nil {
			os.WriteFile(filepath.Join(mqDir, microAppMQType+".go"), []byte(mqGo), 0644)
			fmt.Printf("  Generated: pkg/mq/%s.go\n", microAppMQType)
		}
	}

	mqYaml := fmt.Sprintf("\n# MQ 配置\nmq:\n  type: %s\n  host: %s\n  port: %s\n  topic: %s\n  user: %s\n  password: %s\n  group: %s\n",
		microAppMQType, microAppMQHost, microAppMQPort, microAppMQTopic, microAppMQUser, microAppMQPassword, microAppMQGroup)

	bffCfgPath := filepath.Join(projectDir, toBffDirName(bffName), "configs", "config.yaml")
	if existing, err := os.ReadFile(bffCfgPath); err == nil {
		os.WriteFile(bffCfgPath, append(existing, []byte(mqYaml)...), 0644)
		fmt.Printf("  Updated BFF config: %s/configs/config.yaml (added mq section)\n", toBffDirName(bffName))
	}

	// 更新 pkg/config/config.go（SRV config.yaml 由 genSrvMainAndConfig 处理）
	configGoPath := filepath.Join(projectDir, "pkg", "config", "config.go")
	if content, err := os.ReadFile(configGoPath); err == nil {
		cfgStr := string(content)
		// 在 Config struct 中添加 MQ 字段（如果不存在）
		if !strings.Contains(cfgStr, "MQ       MQConfig") {
			cfgStr = strings.Replace(cfgStr,
				"Registry RegistryConfig `yaml:\"registry\"`",
				"Registry RegistryConfig `yaml:\"registry\"`\n\tMQ       MQConfig       `yaml:\"mq\"`",
				1,
		...
		// 在 TracingConfig 之后添加 MQConfig struct（如果不存在）
		if !strings.Contains(cfgStr, "type MQConfig struct") {
			cfgStr = strings.Replace(cfgStr,
				"ServiceName string `yaml:\"service_name\"`\n}\n",
				"ServiceName string `yaml:\"service_name\"`\n}\n\n// MQConfig MQ 配置\ntype MQConfig struct {\n\tType     string `yaml:\"type\"`\n\tHost     string `yaml:\"host\"`\n\tPort     string `yaml:\"port\"`\n\tTopic    string `yaml:\"topic\"`\n\tUser     string `yaml:\"user\"`\n\tPassword string `yaml:\"password\"`\n\tGroup    string `yaml:\"group\"`\n}\n",
				1,
		...
		os.WriteFile(configGoPath, []byte(cfgStr), 0644)
		fmt.Println("  Updated: pkg/config/config.go (added MQConfig)")
	}
}

// genTraceConfig 生成链路追踪支持
func genTraceConfig(projectDir, bffName string, modules []string) {
	fmt.Println("Generating tracing support...")

	tracingDir := filepath.Join(projectDir, "pkg", "tracing")
	os.MkdirAll(tracingDir, 0755)

	// tracing.go, interceptors.go, middleware.go are already generated in genTracingTemplates
	// This function only updates config.yaml and logs

	traceYaml := fmt.Sprintf("\n# 链路追踪配置\ntracing:\n  host: %s\n  port: %s\n  service_name: %s\n",
		microAppTraceHost, microAppTracePort, microAppTraceSvcName)

	bffCfgPath := filepath.Join(projectDir, toBffDirName(bffName), "configs", "config.yaml")
	if existing, err := os.ReadFile(bffCfgPath); err == nil {
		os.WriteFile(bffCfgPath, append(existing, []byte(traceYaml)...), 0644)
		fmt.Printf("  Updated BFF config: %s/configs/config.yaml (added tracing section)\n", toBffDirName(bffName))
	}

	// 更新 pkg/config/config.go（SRV config.yaml 由 genSrvMainAndConfig 处理）
	// TracingConfig struct 已在模板中定义，无需重复添加
}
