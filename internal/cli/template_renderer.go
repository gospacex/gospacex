...
// ProtoColInfo proto 字段信息（用于联表查询响应消息）
type ProtoColInfo struct {
	Name      string // 字段名 (snake_case)
	ProtoType string // proto 类型 (int64, string, double, etc.)
	Comment   string // 字段注释
	Number    int    // proto 字段编号
}

// JoinServiceMethod Join Service RPC 方法定义
type JoinServiceMethod struct {
	MethodName string // RPC 方法名 (如 "FindJoinProduct2ProductAttr")
	ReqType    string // 请求类型 (如 "GetProduct2ProductAttrReq")
	RespType   string // 响应类型 (如 "GetProduct2ProductAttrResp")
}

// JoinMessageInfo Join 消息定义
type JoinMessageInfo struct {
	Name     string         // 消息名 (如 "GetProduct2ProductAttrResp")
	Fields   []ProtoColInfo // 消息字段列表
	ListName string         // List 消息名 (一对多时使用，如 "Product2ProductAttrList")
	ItemName string         // Item 消息名 (一对多时使用，如 "Product2ProductAttrItem")
}
...
type TemplateData struct {
	...
	Otel bool // 是否启用 OpenTelemetry

	// Join 相关字段
	HasJoin              bool   // 是否启用联表查询
	JoinHandlerMethodName string // Handler 方法名（如 "FindJoinProduct2ProductAttr"）
	JoinServiceMethodName string // Service 方法名（如 "JoinProduct2ProductAttr"）
	JoinReqType          string // 请求类型（如 "GetProduct2ProductAttrReq"）
	JoinRespType         string // 响应类型（如 "GetProduct2ProductAttrResp"）
	JoinModelName        string // Model 名称（如 "JoinProduct2ProductAttr"）
	JoinLeftEntityName   string // 左表实体名截断（如 "Product"）
	JoinRightEntityName  string // 右表实体名截断（如 "Attr"）
	JoinLeftColumns      []ProtoColInfo // 左表字段（用于生成响应消息）
	JoinRightColumns     []ProtoColInfo // 右表字段（用于生成响应消息）
	JoinLeftEntityUpper  string // 左表实体名大写（如 "StoreProduct"）
	JoinRightEntityUpper string // 右表实体名大写（如 "StoreProductAttr"）
	JoinStyle            string // 联表类型：1t1, 1tn, nt1
	JoinServiceMethods   []JoinServiceMethod // Join Service RPC 方法列表（用于 service 模板）
	JoinMessages         []JoinMessageInfo   // Join 消息定义列表（用于 message 模板）
}
...
func buildTemplateData(module string, columns []ColumnInfo, bffName string, tableName string, srvPort int, entityNames ...string) TemplateData {
	...
	if protoImportPath == "" {
		protoImportPath = module // 默认用 module 名作为 proto import 路径
	}
